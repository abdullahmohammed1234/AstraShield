...
export const debrisApi = {
  capture: (snapshotDate) => api.post('/debris/capture', { snapshotDate }),
  getTrends: (orbitalShell, startDate, endDate) => 
    api.get(`/debris/trends?orbitalShell=${orbitalShell}${startDate ? `&startDate=${startDate}` : ''}${endDate ? `&endDate=${endDate}` : ''}`),
  getHistorical: (months = 12) => api.get(`/debris/historical?months=${months}`),
  getDensity: (orbitalShell, resolution = 50) => 
    api.get(`/debris/density?orbitalShell=${orbitalShell}&resolution=${resolution}`),
  getGrowthRate: (orbitalShell, periodMonths = 12) => 
    api.get(`/debris/growth-rate?orbitalShell=${orbitalShell}&periodMonths=${periodMonths}`),
  getStatistics: () => api.get('/debris/statistics'),
  getCurrent: (orbitalShell) => api.get(`/debris/current${orbitalShell ? `?orbitalShell=${orbitalShell}` : ''}`)
};

export const breakupApi = {
  simulate: (params) => api.post('/breakup/simulate', params),
  simulateCollision: (params) => api.post('/breakup/collision', params),
  getEvents: (filters = {}) => {
    const queryParams = new URLSearchParams(filters).toString();
    return api.get(`/breakup/events${queryParams ? `?${queryParams}` : ''}`);
  },
  getEvent: (eventId) => api.get(`/breakup/events/${eventId}`),
  getDispersion: (eventId) => api.get(`/breakup/dispersion/${eventId}`),
  getCascadeProbability: (debrisCount, orbitalShell) => 
    api.get(`/breakup/cascade-probability?debrisCount=${debrisCount}&orbitalShell=${orbitalShell}`)
};

export const kesslerApi = {
  project: (params) => api.post('/kessler/project', params),
  getProjections: (filters = {}) => {
    const queryParams = new URLSearchParams(filters).toString();
    return api.get(`/kessler/projections${queryParams ? `?${queryParams}` : ''}`);
  },
  getProjection: (projectionId) => api.get(`/kessler/projections/${projectionId}`),
  getShellProjection: (projectionId, orbitalShell, year) => 
    api.get(`/kessler/shell/${projectionId}/${orbitalShell}${year ? `?year=${year}` : ''}`),
  getSensitivity: (projectionId) => api.get(`/kessler/sensitivity/${projectionId}`),
  getInitialConditions: () => api.get('/kessler/initial-conditions'),
  getRiskAnalysis: () => api.get('/kessler/risk-analysis')
};
