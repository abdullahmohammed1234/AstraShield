...
class MLRiskPredictor {
  constructor() {
    ...
    this.anomalyCache = null;
    this.anomalyCacheTimestamp = null;
  ...
  async detectAllAnomalies(limit = 100) {
    ...
    if (this.anomalyCache && this.anomalyCacheTimestamp) {
      const cacheAge = Date.now() - this.anomalyCacheTimestamp;
      if (cacheAge < 10 * 60 * 1000) {
        return {
          ...this.anomalyCache,
          source: 'cache',
          generatedAt: this.anomalyCacheTimestamp
        };
      }
    }
    ...
    for (let i = 0; i < satellites.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(sat => this.detectAnomalies(sat.noradCatId))
      );
      
      batchResults.forEach((anomalyResult, idx) => {
        const sat = batch[idx];
    ...
    // Cache results
    this.anomalyCache = results;
    this.anomalyCacheTimestamp = results.generatedAt;
